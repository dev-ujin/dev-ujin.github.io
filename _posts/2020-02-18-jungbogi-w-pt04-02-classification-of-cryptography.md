...
- 치환 암호 : 평문의 문자를 다른 문자로 교환하는 규칙.
- 평문에 사용된 문자의 집합과 암호문에 사용된 문자의 집합이 다를 수 있음.
- 일대일 대응일 필요가 없음.

##### 전치 암호(Transposition Cipher)
- 전치 암호 : 문자 집합 내에서 자리를 바꾸는 규칙.
- 평문에 사용된 문자의 집합과 암호문에 사용된 문자의 집합이 동일.
- 평문에 사용된 문자와 암호문에 사용된 문자가 일대일 대응 규칙을 가짐.

#### 2. 블록 암호와 스트림 암호
##### 블록 암호(Block Cipher)
- 블록 암호 : 어느 특정 비트 수의 집합을 한 번에 처리하는 암호 알고리즘을 총칭.
- 이 집합을 블록이라고 하며, 블록 길이(Block Length)는 블록의 비트 수.
- 암호화 방법 : 평문을 일정한 크기의 블록으로 잘라낸 후 암호화 알고리즘을 적용.
- 스트림 암호와는 다르게 Round를 사용하고, 반복적으로 암호화 과정을 수행해 암호화 강도를 높임.

##### 스트림 암호(Stream Cipher)
- 스트림 암호 : 한번에 1비트 혹은 1바이트의 데이터 흐름(스트림)을 순차적으로 처리해가는 암호 알고리즘을 총칭.
...
- 클럭의 주기에 맞추어 여러 개의 시프트 레지스터 내용이 하나씩 시프트되고, 동시에 출력 값과 시프트 레지스터의 배타적 논리 연산(exclusive OR)값이 시프트 레지스터의 입력으로 인가되는 회로.
- 의사랜덤 시퀀스를 생성하거나 암호 등의 회로에 응용.

#### 3. 링크 암호와와 종단간 암호화
##### 링크 암호화(Link Encryption)
- 링크 암호화 : 모든 정보가 암호화되고 각 **홉(hop)**에서 해독되는 방식.
- 라우터는 패킷의 헤더 부분을 해독하여 라우팅과 주소 정보를 읽고 그것을 다시 암호화하여 진행방향으로 보냄.

##### 종단간 암호화(End-to-End Encryption)
- 종단간 암호화 : 헤더와 트레일러는 암호화되지 않아 각 홉에서 해독하고 암호화할 필요가 없는 방식.
