...
- `밥(Bob)` : 메시지를 수신하는 모델.
- `이브(Eve)` : 도청자(Eavesdropper)의 의미. 소극적인 공격자로 앨리스와 밥 사이에서 이루어지는 통신을 도청하긴 하지만 메시지를 수정하지는 못함.
- `맬로리(Mallory)` : 악의를 가진(Malicious) 공격자를 의미. 앨리스와 밥 사이에서 통신 중인 메시지를 수정하고 재전송할 수 있는 능력을 가짐.
- `트렌트(Trent)` : 중재자(Trusted Arbitrator)를 의미. 중립적인 위치에 있는 제3자.
- `빅터(Victor)` : 검증자(Verifier)를 의미. 의도된 거래나 통신이 실제로 발생했음을 검증할 때 등장.
